class ModelController < ApplicationController
  def show
    @genome_model = Genome::Model.find_for_model_page(params[:id])
    ...
    inputs_for_this_model = @genome_model.model_inputs.inject({}) do |input_hash, model_input|
      input_hash["#{model_input.name}"] = model_input
      input_hash
    end

    @delta_inputs_for_build = @builds.inject({}) do |input_hash, build|
      ...
      build.build_inputs.each do |build_input|
